function sessionSummary = analyze_choiceRTlogDataWeeklyPorts(implantSide,port)
parentFolder=pwd;
graphFolder=[parentFolder(1:end-7) 'testgraphs'];
direct=dir;
count=0;
    ...
    if length(fieldnames(logData))>=27 && length(logData.Attempt)>20
        ...
        logDataWeek.MT= [logData.MT];
            ...
            if strcmpi(implantSide, 'left')
                sessionSummary.targetContra = sessionSummary.targetRight & logDataWeek.Center==port;
                sessionSummary.targetIpsi   = ~sessionSummary.targetRight & logDataWeek.Center==port;
                
                sessionSummary.moveContra   = sessionSummary.moveRight & logDataWeek.Center==port;
                sessionSummary.moveIpsi     = ~sessionSummary.moveRight & logDataWeek.SideNP > 0 & logDataWeek.Center==port;
                
            else
                sessionSummary.targetContra = ~sessionSummary.targetRight & logDataWeek.Center==port;
                sessionSummary.targetIpsi   = sessionSummary.targetRight & logDataWeek.Center==port;
                
                sessionSummary.moveContra   = ~sessionSummary.moveRight & logDataWeek.SideNP > 0 & logDataWeek.Center==port;
                sessionSummary.moveIpsi     = sessionSummary.moveRight & logDataWeek.Center==port;
            end
            
            correctTargetContra = sessionSummary.correct & sessionSummary.targetContra & logDataWeek.Center==port;
            correctTargetIpsi   = sessionSummary.correct & sessionSummary.targetIpsi & logDataWeek.Center==port;
            
            
            completeTargetContra = sessionSummary.complete & sessionSummary.targetContra & logDataWeek.Center==port;
            completeTargetIpsi   = sessionSummary.complete & sessionSummary.targetIpsi & logDataWeek.Center==port;
            ...
            sessionSummary.acc(3) = sum(sessionSummary.correct) / sum(logDataWeek.Center==port);
...
%             AccuracyCenter4(count)=sessionSummaryCenter.acc(3);
           ...
           % new plots for attempts etc.
            ...
            allAttempt(count)= length(sessionSummary.correct & logDataWeek.Center==port);
            
            ipsiComplete(count)= sum(completeTargetIpsi);
            contraComplete(count)= sum(completeTargetContra);
            allComplete(count)= sum(sessionSummary.complete & logDataWeek.Center==port);
            
            ipsiCorrect(count)= sum(correctTargetIpsi);
            contraCorrect(count)= sum(correctTargetContra);
            allCorrect(count)= sum(sessionSummary.correct & logDataWeek.Center==port);
           ...
           % accuracy new plots 
            ...
            for i_outcome = 1 : 6
                plotMatrix(1, i_outcome) = sum(logDataWeek.outcome == outcomeList(i_outcome) & ...
                    sessionSummary.targetIpsi & logDataWeek.Center==port);
                plotMatrix(2, i_outcome) = sum(logDataWeek.outcome == outcomeList(i_outcome) & ...
                    sessionSummary.targetContra & logDataWeek.Center==port);
                plotMatrix(3, i_outcome) = sum(logDataWeek.outcome == outcomeList(i_outcome) & logDataWeek.Center==port);
            end
            ...
            plotMatrix(3,:) = plotMatrix(3,:) ./ sum(logDataWeek.Center==port);
       ...
       if count==5
        ...
        mls = sprintf('%s\n%s\n%s',A{1,1},A{1,2},A{1,3});
       name=fname(1).name ;
       textString{1} = [name(1:5) '(' logDataWeek.combined ')_LogAnalysisRTMTPort' int2str(port)];
       %# centimeters units
       legend('Day1','Day2','Day3','Day4','Day5','Location','NorthEast');
       textLoc = [];
...
mls = sprintf('%s\n%s\n%s',A{1,1},A{1,2},A{1,3});
        ...
        textString{1} = [name(1:5) '(' logDataWeek.combined ')_LogAnalysisOutcomePort' int2str(port)];    
       set(gcf, 'PaperPosition', [xMargin yMargin xSize ySize]); %Position the plot further to the left and down. Extend the plot to fill entire paper.
       set(gcf, 'PaperSize', [X Y]); %Keep the same paper size
       figureSize = get(gcf,'Position');
       uicontrol('Style','text',...
