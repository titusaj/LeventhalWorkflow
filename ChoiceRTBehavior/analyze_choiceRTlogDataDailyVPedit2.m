function sessionSummary = analyze_choiceRTlogDataDailyVPedit2(varargin)
%
...
% if ispc   % work around for weird sizing on PCs
%     figProps.panelHeight = figProps.panelHeight * 26.4583 / (11*2.54);
%     figProps.rowSpacing = figProps.rowSpacing * 26.4583 / (11*2.54);
%     figProps.height = figProps.height * 26.4583 / (11*2.54);
%     figProps.topMargin = figProps.topMargin * 26.4583 / (11*2.54);
% end
%     
...
%     topLevelDir = sql_findNASpathVPedit(ratID, ...
% %                               'hostip', hostIP, ...
% %                               'user', user, ...
% %                               'password', password, ...
% %                               'dbname', dbName, ...
...
%     ii = iDir + 4;
    ...
    textString{1} = [direct(ii).name '_LogAnalysis_test.pdf'];
...
%                                'color', 'w','PaperUnits','normalized','PaperType','usletter')
%                                 , ...
%                                 'position',[1 1 X Y]);
        ...
        set(gca,'xticklabel',{'ipsi','contra','all'}, ...
            'ylim',[0 1]);
        [h_leg,h_objects,h_plot,leg_strings] = legend('cor','fs','ws','lh','mh','wrong','Location','NorthEast');
        legPos = zeros(1,4);
        ...
        set(gca,'Units',figUnits,'Position',figPosition);%[0 0 figPosition(3) figPosition(4)]);
        rectangle('position',[0 0 figPosition(3) figPosition(4)],'edgecolor','none','facecolor','none');
        
        plotPosition = get(gca, 'Position');
        if ismac
            textBot = plotPosition(4)-2.54;
        elseif ispc
            textBot = plotPosition(4)-3;
        end
        textLeft = 5;
...
%             'PaperUnits','inches', ...
%             'Paperposition',[1 1 8.5 11])
        
        cd(graphFolder);
        figure(h_fig);
        export_fig(graphName,'-pdf','-q101','-painters', '-nocrop');
%          saveas(gcf, graphName, 'pdf')
        close(h_fig)
        cd(parentFolder);
    end
    cd(parentFolder);
